...
import { createOffsetAllocator, type OAHandle, oaAllocate, oaFree, oaStorageReport } from './offset-allocator';
...
function meshChunk(world: World, chunk: Chunk): MeshData | null {
            ...
            for (let y = -1; y < CHUNK_SIZE; y++) {
                ...
                for (let dir = 0; dir < 3; dir++) {
                    ...
                    const geom =
                        ...
                        0;
                    const attr =
                        ((color & 0x3f) | (ao2(ao00) << 6) | (ao2(ao10) << 8) | (ao2(ao11) << 10) | (ao2(ao01) << 12)) >>> 0;
    ...
    const ox = chunk.cx * CHUNK_SIZE,
        oy = chunk.cy * CHUNK_SIZE,
        oz = chunk.cz * CHUNK_SIZE;
    return {
...
const cullCompute = Fn(() => {
    const id = globalId.x;
    If(id.greaterThanEqual(u32(MAX_DRAWS)), () => {
        Return();
    });
...
const faceU = wgslFn(
    `fn faceU(f:u32)->vec3f { var a=array<vec3f,6>(vec3f(0,1,0),vec3f(0,0,1),vec3f(0,0,1),vec3f(1,0,0),vec3f(1,0,0),vec3f(0,1,0)); return a[f]; }`,
    { output: d.vec3f },
);
const faceV = wgslFn(
    `fn faceV(f:u32)->vec3f { var a=array<vec3f,6>(vec3f(0,0,1),vec3f(0,1,0),vec3f(1,0,0),vec3f(0,0,1),vec3f(0,1,0),vec3f(1,0,0)); return a[f]; }`,
    { output: d.vec3f },
);
const faceN = wgslFn(
    `fn faceN(f:u32)->vec3f { var a=array<vec3f,6>(vec3f(1,0,0),vec3f(-1,0,0),vec3f(0,1,0),vec3f(0,-1,0),vec3f(0,0,1),vec3f(0,0,-1)); return a[f]; }`,
    { output: d.vec3f },
);
// corner id (0..3) per (diagFlip, vertexInTri 0..5) — two triangulations
const cornerIdx = wgslFn(
    `fn cornerIdx(flip:u32, v:u32)->u32 { var a=array<u32,12>(0u,1u,2u,0u,2u,3u,0u,1u,3u,1u,2u,3u); return a[flip*6u+v]; }`,
    { output: d.u32 },
);
// corner (0..3) -> unit (u,v): 0:(0,0) 1:(1,0) 2:(1,1) 3:(0,1)
const cornerUV = wgslFn(
    `fn cornerUV(c:u32)->vec2f { var a=array<vec2f,4>(vec2f(0,0),vec2f(1,0),vec2f(1,1),vec2f(0,1)); return a[c]; }`,
    { output: d.vec2f },
);
const palette = wgslFn(
    `fn palette(c:u32)->vec3f { var a=array<vec3f,6>(vec3f(0.50,0.50,0.52),vec3f(0.42,0.30,0.18),vec3f(0.30,0.55,0.22),vec3f(0.55,0.55,0.50),vec3f(0.92,0.94,0.97),vec3f(0.30,0.45,0.70)); return a[min(c,5u)]; }`,
    { output: d.vec3f },
);
...
function uploadChunk(chunkIdx: number): void {
    ...
    if (!handle) {
        console.warn('voxels: arena out of space');
        return;
    }
    const slot = allocSlot();
    if (slot < 0) {
        oaFree(arena, handle);
        console.warn('voxels: out of draw slots');
        return;
    }
...
async function main() {
    ...
    view.clearColor = [0.53, 0.8, 0.92, 1];

    const statsEl = document.createElement('div');
    statsEl.style.cssText =
        'position:fixed;top:8px;left:8px;background:rgba(0,0,0,0.7);color:#fff;padding:8px 12px;font:12px/1.5 monospace;border-radius:4px;pointer-events:none;z-index:100;';
    ...
    function getRayFromScreen(clientX: number, clientY: number) {
        ...
        const vx = ndcX * tanHalfFov * camera.aspect,
            vy = ndcY * tanHalfFov,
            vz = -1;
        ...
        return {
            ox: camera.position[0],
            oy: camera.position[1],
            oz: camera.position[2],
            dx: dx / len,
            dy: dy / len,
            dz: dz / len,
        };
    ...
    function update() {
        ...
        cullPass.dispatch(cullCompute, [Math.ceil(MAX_DRAWS / 64), 1, 1]);
        cullPass.end();
