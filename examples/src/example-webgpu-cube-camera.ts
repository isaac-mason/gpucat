...
legend.innerHTML =
    'mirror sphere reflecting a labeled room. each wall: WHITE marker = top-left, BLACK = top-right.<br>' +
    '+X red &nbsp; -X green &nbsp; +Y blue &nbsp; -Y yellow &nbsp; +Z magenta &nbsp; -Z cyan';
...
async function updateFaceStrip(): Promise<void> {
    ...
    try {
        ...
        const faces = await Promise.all([0, 1, 2, 3, 4, 5].map((face) => read(renderer, cubeRT, { layer: face })));
