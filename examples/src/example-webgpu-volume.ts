...
const valueNoise = Fn(
    (p: Node<d.vec3f>) => {
        ...
        const u = f
            .mul(f)
            .mul(vec3(f32(3), f32(3), f32(3)).sub(f.mul(f32(2))))
            .toVar('u'); // smoothstep weights
...
const fillVolume = Fn(() => {
    ...
    const sp = p
        .mul(f32(4.5))
        .add(vec3(time.mul(f32(0.25)), f32(0), time.mul(f32(0.12))))
        .toVar('sp');
...
const material = createMaterial({ vertex: clipPos, fragment: raymarch(vWorld) });
...
view.clearColor = [BG[0], BG[1], BG[2], 1];
...
function update() {
    ...
    const fillPass = f.compute();
    fillPass.dispatch(fillVolume, [N / WG, N / WG, N / WG]);
    fillPass.end();
