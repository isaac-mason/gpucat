 ...
 */
export function createBindingsState(layoutCache: BindGroupLayoutCache): BindingsState {
    return {
        layoutCache,
        data: new WeakMap(),
    };
...
export function initRenderBindings(state: BindingsState, renderObject: RenderObject, device: GPUDevice): void {
    const nodeState = renderObject.nodeBuilderState;
    if (!nodeState) return;
...
export function getRenderBindGroupLayouts(state: BindingsState, renderObject: RenderObject): GPUBindGroupLayout[] {
    const nodeState = renderObject.nodeBuilderState;
    if (!nodeState) return [];
...
function initBindGroup(state: BindingsState, bindGroup: BindGroup, device: GPUDevice, visibility: GPUShaderStageFlags): void {
    const data = getData(state, bindGroup);
        ...
        switch (binding.kind) {
            ...
            case 'storage':
                entries.push({
                    ...
                    visibility,
                    buffer: {
                        type: binding.entry.access === 'read_write' ? 'storage' : 'read-only-storage',
                    },
                });
                break;
...
function updateTextureBinding(textureCache: TextureCache, device: GPUDevice, binding: TextureBinding, data: BindGroupData): void {
