...
function writeQuad(
    ...
    side[vi + 0] = -1; side[vi + 1] = -1;
    side[vi + 2] =  1; side[vi + 3] =  1;

    uv[(vi + 0) * 2] = 0; uv[(vi + 0) * 2 + 1] = 0;
    uv[(vi + 1) * 2] = 1; uv[(vi + 1) * 2 + 1] = 0;
    uv[(vi + 2) * 2] = 0; uv[(vi + 2) * 2 + 1] = 1;
    uv[(vi + 3) * 2] = 1; uv[(vi + 3) * 2 + 1] = 1;

    const ii = (segmentOffset + s) * 6;
    indices[ii + 0] = vi + 0; indices[ii + 1] = vi + 1; indices[ii + 2] = vi + 2;
    indices[ii + 3] = vi + 1; indices[ii + 4] = vi + 3; indices[ii + 5] = vi + 2;
}
...
function writePolylineSegments(
    ...
    for (let s = 0; s < segmentCount; s++) {
        ...
        writeQuad(s, segmentOffset,
            src[s * 3], src[s * 3 + 1], src[s * 3 + 2],
            src[ni * 3], src[ni * 3 + 1], src[ni * 3 + 2],
            instanceStart, instanceEnd, side, uv, indices);
    }
}
...
function writeSegmentPairs(
    ...
    for (let s = 0; s < segmentCount; s++) {
        const si = s * 2;
        writeQuad(s, segmentOffset,
            src[si * 3], src[si * 3 + 1], src[si * 3 + 2],
            src[(si + 1) * 3], src[(si + 1) * 3 + 1], src[(si + 1) * 3 + 2],
            instanceStart, instanceEnd, side, uv, indices);
    }
}
...
function lineVertex(lineWidthNode: Node<d.f32>, worldUnits = false): Node<d.vec4f> {
    ...
    const toClip = (p: Node<d.vec3f>) =>
        mul(cameraProjectionMatrix, mul(cameraViewMatrix, mul(modelWorldMatrix, vec4f(p, 1)))) as Node<d.vec4f>;
    ...
    const width = worldUnits
        ? mul(lineWidthNode, clipPos.w as unknown as Node<d.f32>)
        : lineWidthNode;
...
function raycastWorldUnits(
    ...
    for (let s = 0; s < n; s++) {
        ...
        _start[0] = starts[vi]; _start[1] = starts[vi+1]; _start[2] = starts[vi+2];
        _end[0]   = ends  [vi]; _end[1]   = ends  [vi+1]; _end[2]   = ends  [vi+2];
        ...
        const isInside = Math.sqrt(
            (point[0]-pointOnLine[0])**2 +
            (point[1]-pointOnLine[1])**2 +
            (point[2]-pointOnLine[2])**2
        ) < lineWidth * 0.5;
...
function raycastScreenSpace(
    ...
    ssOrigin[3] = 1.0;
    applyMat4Vec4(ssOrigin as unknown as Float64Array, camera.matrixWorldInverse,
        ssOrigin[0], ssOrigin[1], ssOrigin[2], ssOrigin[3]);
    applyMat4Vec4(ssOrigin as unknown as Float64Array, proj,
        ssOrigin[0], ssOrigin[1], ssOrigin[2], ssOrigin[3]);
    ...
    for (let s = 0; s < n; s++) {
        ...
        const ax = starts[vi], ay = starts[vi+1], az = starts[vi+2];
        const bx = ends  [vi], by = ends  [vi+1], bz = ends  [vi+2];
        ...
        _start4[0] *= sInvW; _start4[1] *= sInvW; _start4[2] *= sInvW;
        _end4[0]   *= eInvW; _end4[1]   *= eInvW; _end4[2]   *= eInvW;
        ...
        if (isInside) {
            ...
            distanceSqToSegment(raycaster.ray.origin, raycaster.ray.direction,
                _start, _end, point, pointOnLine);
...
function raycastLine(
    ...
    if (material.worldUnits) {
        if (geometry.boundingBox) {
            ...
            worldBox[0] -= m; worldBox[1] -= m; worldBox[2] -= m;
            worldBox[3] += m; worldBox[4] += m; worldBox[5] += m;
...
export class LineSegments extends Mesh {
    ...
    override raycast(raycaster: Raycaster, intersects: Intersection[]): void {
        raycastLine(this, this.material as LineMaterial, this.geometry as LineSegmentsGeometry, raycaster, this.threshold, intersects);
    }
}
