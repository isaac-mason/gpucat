...
type ControllerForType<V> = V extends number
    ? NumberController
    : V extends boolean
      ? BooleanController
      : V extends string
...
export class GUI {
    ...
    addColor<T extends object, K extends keyof T & string>(object: T, property: K, rgbScale = 1): ColorController {
    ...
    controllersRecursive(): Controller[] {
        ...
        this.folders.forEach((f) => {
            result = result.concat(f.controllersRecursive());
        });
        return result;
    }
    ...
    foldersRecursive(): GUI[] {
        ...
        this.folders.forEach((f) => {
            result = result.concat(f.foldersRecursive());
        });
        return result;
    }
