...
export class RendererInspector extends InspectorBase {
    ...
    private _pendingMaps = new Set<Promise<unknown>>();
    ...
    protected disposeTimestampGpu(): Promise<void> {
        ...
        this._readbackPool = [];
        const drained =
            querySet || resolveBuffer || readbackPool.length > 0
                ? this.drainThenDestroy(() => {
                      querySet?.destroy();
                      // GPUBuffers don't always expose destroy on all browsers; guard.
                      if (resolveBuffer?.destroy) resolveBuffer.destroy();
                      for (const b of readbackPool) b.destroy?.();
                  })
                : Promise.resolve();
     ...
     */
    private _acquireReadback(device: GPUDevice, size: number): GPUBuffer | null {
        ...
        if (!this._readbackSaturatedLogged) {
            ...
            this.log.warn(
                `[inspector] timestamp readback pool saturated at ${READBACK_POOL_CAP} buffers; dropping this frame's GPU timings (readbacks are landing slower than ${READBACK_POOL_CAP} frames).`,
            );
        }
        return null;
    }
