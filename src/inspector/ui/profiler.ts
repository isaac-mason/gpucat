...
`;
    ...
    toggleMaximize(): void {
        if (this.panel.classList.contains('maximized')) {
            ...
            this.maximizeBtn.innerHTML =
                '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="8" width="12" height="12" rx="2" ry="2"></rect><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"></path></svg>';
        }
    }
    ...
    addBuiltinTab(tab: Tab): void {
        ...
        if (!tab.isVisible) {
            ...
            const hasVisibleBuiltinButtons = Array.from(this.builtinTabsContainer.querySelectorAll('.builtin-tab-btn')).some(
    ...
    detachTab(tab: Tab, x: number, y: number): void {
        ...
        const tabIdsInOrder = allButtons
            .map((btn) => Object.keys(this.tabs).find((id) => this.tabs[id].button === btn))
        ...
        if (this.activeTabId === tab.id) {
            ...
            const remainingTabs = tabIdsInOrder.filter(
                (id) => id !== tab.id && !this.tabs[id].isDetached && this.tabs[id].isVisible,
            );
    ...
    setupDetachedWindowDrag(windowPanel: HTMLDivElement, header: HTMLDivElement, tab: Tab): void {
        ...
        const onDragMove = (e: PointerEvent) => {
            ...
            const pw = windowPanel.offsetWidth,
                ph = windowPanel.offsetHeight;
            const hw = pw / 2,
                hh = ph / 2;
    ...
    setupDetachedWindowResize(
        windowPanel: HTMLDivElement,
        resizerTop: HTMLDivElement,
        resizerRight: HTMLDivElement,
        resizerBottom: HTMLDivElement,
        resizerLeft: HTMLDivElement,
        resizerCorner: HTMLDivElement,
    ): void {
        const minWidth = 250;
        const minHeight = 150;
    ...
    setPosition(targetPosition: string): void {
        ...
        if (targetPosition === 'right') {
            ...
            this.floatingBtn.innerHTML =
                '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><path d="M3 15h18"></path></svg>';
